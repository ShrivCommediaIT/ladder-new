...
const PlayerStatusToggle = ({ player, user, viewOnly, showTokenStatus }) => {
  ...
  if (viewOnly) {
    return (
      <div className="flex items-center gap-1.5 py-1">
        ...
        {showTokenStatus && player?.token_status && (
  ...
  return (
    <div className="flex items-center gap-2">
      <div 
        className="flex items-center gap-1 sm:gap-2 bg-black/20 px-1 sm:px-2 py-0.5 sm:py-1 rounded-full border border-white/5"
      ...
      >
        <Switch
          ...
          className="data-[state=checked]:bg-emerald-500 data-[state=unchecked]:bg-zinc-600 scale-75 sm:scale-100 origin-left"
        />
        <Label 
          htmlFor={`status-${player.id}`}
          className={`text-[9px] sm:text-xs font-bold uppercase transition-colors whitespace-nowrap ${

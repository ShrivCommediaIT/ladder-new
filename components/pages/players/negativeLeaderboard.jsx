...
const PlayerCard = ({
  ...
  const getFirstScoreValue = (...values) =>
    values.find((value) => value !== null && value !== undefined && value !== "");

  const getNegativeScoreInSeconds = (value) => {
    if (value === null || value === undefined || value === "") return 0;

    const numericValue = Number(value);
    if (!Number.isNaN(numericValue)) return numericValue;

    const convertedValue = Number(convertTimeToSeconds(value));
  ...
  const getScoreBySkillNumber = (scores, skills, skillNumber) => {
    ...
    const rawNegativeScore = getFirstScoreValue(
      scoreObj?.negative_ladder_bestscore,
      scoreObj?.negative_ladder_score,
      scoreObj?.best_score,
      scoreObj?.input_score
    );
    const score = getNegativeScoreInSeconds(rawNegativeScore);
    ...
    return { witnessBy, score, displayScore, target, isTargetAchieved, input_score: rawNegativeScore };
  };
...
const NegativeLeaderboard = ({ ladderId: propLadderId, onPlayerAdded }) => {
  ...
  const [selectedPositiveFilter, _setSelectedPositiveFilter] = useState(0);
    ...
    if (typeof window !== "undefined") {
      ...
      if (storedUser) {
        try {
          ...
          if (parsedUser?.id) setCurrentUserId(Number(parsedUser.id));
        } catch (err) {
      ...
      <LadderPageLayout
        ...
        sidebar={
          <InfoSection
            ...
            resetDescription="This will reset the current negative ladder data."
        ...
        <DialogContent showCloseButton={false} className="bg-transparent border-none shadow-none flex items-center justify-center">
          <BasicLeaderboardShort
            ...
            onSkillsUpdated={(_skillNo) => {
              ...
              setAgeFilterResetSignal((p) => p + 1);
              setLocalWitnessBy(0);
              _setSelectedPositiveFilter(_skillNo);

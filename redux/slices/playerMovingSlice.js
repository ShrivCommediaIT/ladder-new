...
// // ====================== 28/11/2025 =========================== working only first section
...
//           success_message:
//             data.message ||
//             (match_status === "beat" ? "Move Successful" : "Result Saved (Lost)"),
//           result: data,
//         };
//       } else {
...
// /* -------------------------------------------------------
//    🔥 3) BEST OF 5 API (BET SUPPORTED) - ✅ FIXED
// ------------------------------------------------------- */
...
//       move_to_rank,
//       move_from_rank,
//       bet = "", // ✅ Default empty string
//     },
//     { rejectWithValue }
//   ) => {
//     try {
//       // ✅ VALIDATION - सभी required fields check करें
...
//       /* movePlayerBestOf5 - ✅ PERFECTLY MATCHED */
//       .addCase(movePlayerBestOf5.pending, (state) => {
//         state.loading = true;
//         state.error = null;
//         state.successMessage = null;
//         state.result = null;
//       })
//       .addCase(movePlayerBestOf5.fulfilled, (state, action) => {
//         state.loading = false;
//         state.successMessage = action.payload.success_message;
//         state.result = action.payload.result;
//       })
//       .addCase(movePlayerBestOf5.rejected, (state, action) => {
//         state.loading = false;
//         state.error = action.payload || "Best of 5 save failed";
//       });
//   },
// });
...
//   "playerMoving/movePlayer",
//   async (
//     { user_id, move_from_user_id, move_to_rank, move_from_rank, ladder_id, match_status, score },
//     { rejectWithValue }
//   ) => {
//     try {
...
// /* -------------------------------------------------------
//    🔥 4) MINI LEAGUE MOVE API
// ------------------------------------------------------- */
// export const moveMiniLeague = createAsyncThunk(
//   "playerMoving/moveMiniLeague",
//   async ({ ladder_id, match_status, user_id, score, move_to_rank, move_from_rank, bet = "", move_from_section, move_to_section, }, { rejectWithValue }) => {
//     try {
//       const requiredFields = { ladder_id, match_status, user_id, move_to_rank, score, move_from_rank, move_from_section, move_to_section, };
//       for (const [key, value] of Object.entries(requiredFields)) {
//         if (!value && value !== 0) return rejectWithValue(`Missing required field: ${key}`);
//       }

//       const params = { ladder_id, match_status, user_id, move_to_rank, score, move_from_rank, bet: bet || "", move_from_section, move_to_section, };
...
//       } else {
//         return rejectWithValue(data.message || "Mini League save failed");
//       }
//     } catch (error) {
//       console.error(" MiniLeague Error:", error.response?.data || error.message);
//       return rejectWithValue(error.response?.data?.message || error.message);
//     }
//   }
// );
